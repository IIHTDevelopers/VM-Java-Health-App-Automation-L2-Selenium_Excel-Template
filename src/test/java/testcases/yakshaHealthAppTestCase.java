...
public class yakshaHealthAppTestCase extends AppTestBase
{
	...
	@Test(priority = 12, groups = {"sanity"}, description="Tick all check boxes then untick them then close that popup")
	public void  tickAllCheckBoxesThenUntickthemThenCloseThatPopup() throws Exception {
		yakshaHealthAppInstance = new yakshaHealthAppPages(driver);
		locatorsFactoryInstance = new LocatorsFactory(driver);
		Assert.assertTrue(yakshaHealthAppInstance.tickallCheckBoxesThenUntickthemThenCloseThatPopup(), "Unable to handle the checkBoxes, please check manually");	
	...
	@Test(priority = 13, groups = {"sanity"}, description="Take screenshot of Doctor page")
	...
	@Test(priority = 14, groups = {"sanity"}, description="Deactivate a user or employee by managing the alert popup")
	...
	@Test(priority = 15, groups = {"sanity"}, description="Verify tooltip of an element which is present inside the Appointment page.")
